...
while True:
    ...
    if(len(conts)==2):
    	#pinchFlag para evitar hacer clics continuos
        if(pinchFlag==1):
            pinchFlag=0
            mouse.release(Button.left)
        # lógica para el gesto abierto, mover el mouse sin hacer clic
        x1,y1,w1,h1=cv2.boundingRect(conts[0])
        x2,y2,w2,h2=cv2.boundingRect(conts[1])
        #dibuja rectangulos sobre los objetos de color verde
        cv2.rectangle(img,(x1,y1),(x1+w1,y1+h1),(255,0,0),2)
        cv2.rectangle(img,(x2,y2),(x2+w2,y2+h2),(255,0,0),2)
        #coordenada central del primer objeto
        cx1=int(x1+w1/2)
        cy1=int(y1+h1/2)
        #coordenada central del segundo objeto
        cx2=int(x2+w2/2)
        cy2=int(y2+h2/2)
        #coordenada central de la linea que conecta ambos puntos
        cx=int((cx1+cx2)/2)
        cy=int((cy1+cy2)/2)
        #dibuja la linea
        ...
        if(pinchFlag==0): 
        	...
        	#mouse.press(Button.left)
        	#dibuja un rectangulo sobre los objetos juntos
            cv2.rectangle(img,(x,y),(x+w,y+h),(255,0,0),2)
            cx=int(x+w/2)
            cy=int(y+h/2)
            #dibuja un circulo sobre los objetos juntos
